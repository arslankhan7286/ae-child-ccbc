<?php if(get_post_type($post->ID) == 'sermon') { ?>
        <?php
            $config = \Ardent\Ccbc\Sermons::getConfig();
            $first_term = get_the_terms($post->ID, 'sermon_category');
            $image_id = \Ardent\Ccbc\Sermons\Term::getTermImage($first_term[0]->term_id);
